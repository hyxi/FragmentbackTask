...
public class ScrollableView extends View {
	...
	protected void init(Context context) {
		...
		mScrollState = new ScrollState(context);
	 ...
	 *            变化率算法对象
	 */
	protected void setInterpolator(Context context, Interpolator interpolator) {
		mScrollState = new ScrollState(context, interpolator);
	}
	 ...
	 *            时间
	 */
	public void smoothScrollTo(int fromX, int fromY, int finalX, int finalY,
			int duration) {
		int dx = finalX - fromX;
		int dy = finalY - fromY;
	...
	public void smoothScrollBy(int fromX, int fromY, int distanceX,
			int distanceY, int duration) {
		/* 先停止当前的滚动 */
		stopScroll();
		/* 设置滚动参数 */
		mScrollState.setScrollParams(fromX, fromY, -distanceX, -distanceY,
				duration);
		/* 启动滚动 */
		startScroll();
	}
	...
	public void smoothScrollBy(int distanceX, int distanceY) {
		...
		stopScroll();
		/* 设置滚动参数 */
		mScrollState.setScrollParams(distanceX, distanceY,
	...
	private void scrollDistanceStep(Step step) {
		smoothScrollBy(step.getDistanceX(), step.getDistanceY(),
				step.getDuration());
	}
